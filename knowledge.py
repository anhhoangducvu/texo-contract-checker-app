# -*- coding: utf-8 -*-
"""
Knowledge base RULE-BASED cho rà soát Hợp đồng TVGS (góc nhìn Bên B).
...
# =============================================================================
# RISK_RULES — phát hiện câu chữ bất lợi cho TVGS bằng mẫu (regex, không phân biệt hoa thường)
# Mỗi rule:
#   id        : mã ngắn
...
#   problem   : vì sao bất lợi cho TVGS
...
RISK_RULES = [
    # --- Chủ đề 2: Hồ sơ HĐ & thứ tự ưu tiên ---
    {
        ...
        "problem": "Khi hồ sơ mâu thuẫn/không rõ, CĐT tự quyết nghĩa là vừa đá bóng vừa thổi còi, ép cách hiểu bất lợi cho TVGS.",
    ...
    # --- Chủ đề 3: Phạm vi (Điều 19 NĐ06) ---
    {
        ...
        "topic": 3, "level": DO,
        "label": "Gán TVGS 'thẩm tra / tối ưu hóa thiết kế'",
        "pattern": r"thẩm tra thiết kế|tối ưu (hóa|hoá) thiết kế|design (review|verification|optimi[sz]ation)|kiểm định thiết kế",
        "problem": "Thẩm tra thiết kế là năng lực RIÊNG, không thuộc phạm vi giám sát thi công; gán cho TVGS làm phình trách nhiệm vượt năng lực/giấy phép.",
    ...
    },
    {
        ...
        "topic": 3, "level": DO,
        "label": "Gán TVGS 'phê duyệt' biện pháp/hồ sơ nhà thầu",
        ...
        "basis": "Điều 19.1.b/c NĐ 06/2021: TVGS kiểm tra và chấp thuận; biện pháp do nhà thầu lập, CĐT phê duyệt.",
    ...
    },
    {
        ...
        "topic": 3, "level": CAM,
        "label": "Gán TVGS 'lập/thu thập hồ sơ nghiệm thu, hoàn công'",
        "pattern": r"(lập|thu thập|sắp xếp|hoàn thiện).{0,30}hồ sơ (nghiệm thu|hoàn công|hoàn thành)|(tvgs|bên b).{0,30}hồ sơ hoàn công",
        "problem": "Lập hồ sơ nghiệm thu/hoàn công là việc của nhà thầu và CĐT; TVGS chỉ kiểm tra và ký xác nhận.",
        "basis": "Điều 13.17 (nhà thầu) và Điều 14.11 (CĐT) NĐ 06/2021; TVGS tham gia nghiệm thu, ký xác nhận.",
        "suggest": "Sửa: TVGS 'kiểm tra, tham gia nghiệm thu, ký xác nhận'; việc lập hồ sơ thuộc nhà thầu/CĐT.",
    },
    ...
    },
    {
        ...
        "problem": "Trách nhiệm liên đới buộc TVGS gánh cả phần lỗi của nhà thầu; khoản 'thiệt hại uy tín' rất khó định lượng và dễ bị thổi phồng.",
        "basis": "BLDS 2015 Điều 587 (mỗi bên chịu phần lỗi của mình); thiệt hại phải xác định được.",
        "suggest": "Chỉ chịu phần lỗi của TVGS; loại bỏ hoặc định lượng cụ thể khoản thiệt hại uy tín/thương hiệu.",
    },
    # --- Chủ đề 18: Tạm ngừng & chấm dứt ---
    {
        ...
        "problem": "Cho phép CĐT đơn phương chấm dứt tùy ý. Nếu không kèm nghĩa vụ thanh toán phần đã làm + chi phí huy động thì TVGS chịu thiệt.",
    ...
    },
    {
        "id": "term_no_b_right",
        "topic": 18, "level": CAM,
        "label": "Thiếu quyền chấm dứt của TVGS",
        "pattern": r"(bên b|tư vấn|consultant).{0,40}(không được|no right to) (đơn phương )?chấm dứt|terminat",
        "problem": "Nếu chỉ CĐT có quyền chấm dứt, TVGS không có lối thoát khi CĐT chậm thanh toán hoặc vi phạm nghiêm trọng.",
        "basis": "Đối xứng quyền (mẫu TT02); BLDS 2015 Điều 428 (đơn phương chấm dứt khi bên kia vi phạm nghiêm trọng).",
        "suggest": "Bổ sung quyền TVGS chấm dứt khi CĐT chậm thanh toán/vi phạm trọng yếu, kèm quyền được thanh toán phần đã làm.",
    },
    # --- Chủ đề 8: Thanh toán ---
    {
        "id": "pay_refuse",
        ...
        "problem": "Cho CĐT quyền từ chối (mất quyền) thanh toán cả khoản chỉ vì thiếu một phần hồ sơ — giam tiền của TVGS.",
    ...
    },
    {
        "id": "pay_no_interest",
        "topic": 8, "level": CAM,
        "label": "Thiếu lãi chậm thanh toán cho TVGS",
        "pattern": r"lãi (chậm|do chậm) (thanh toán|trả)|interest on late payment|default interest|lãi suất quá hạn",
        "problem": "(Phát hiện khi CÓ nhắc) — cần kiểm tra lãi chậm thanh toán có ĐỐI XỨNG cho TVGS không, hay chỉ áp một chiều.",
        "basis": "Mẫu TT02 buộc CĐT chậm thanh toán phải trả lãi quá hạn; BLDS Điều 357.",
        "suggest": "Bảo đảm có lãi chậm thanh toán áp cho CĐT khi chậm trả TVGS; bỏ điều kiện 'mất quyền lãi nếu không thông báo trong X ngày'.",
    },
    # --- Chủ đề 10: Bảo hiểm ---
    {
        "id": "ins_professional",
        ...
        "problem": "Pháp luật KHÔNG bắt buộc bảo hiểm trách nhiệm nghề nghiệp đối với TVGS (khác thiết kế/khảo sát); buộc mua làm tăng chi phí.",
        "basis": "NĐ về bảo hiểm trong hoạt động xây dựng: bảo hiểm TNN bắt buộc cho khảo sát/thiết kế, không cho giám sát; BHXH đã gồm BH tai nạn LĐ.",
    ...
    },
    {
        ...
        "problem": "Buộc TVGS làm ngoài giờ/ngày lễ không giới hạn và không bù chi phí, đẩy chi phí nhân công sang TVGS.",
    ...
    # --- Chủ đề 7: Tạm ứng / giữ lại ---
    {
        ...
        "suggest": "Quy định rõ thời điểm/điều kiện hoàn trả phần giữ lại (vd: sau nghiệm thu hoàn thành dịch vụ TVGS).",
    },
    # --- Chủ đề 9: Bảo lãnh ---
    {
    ...
    # --- Chủ đề 20: Chuyển nhượng ---
    {
        ...
        "problem": "(Phát hiện khi CÓ) — kiểm tra điều khoản chuyển nhượng có đối xứng không; thường CĐT được chuyển còn TVGS thì không.",
        "basis": "BLDS 2015 Điều 365 (chuyển giao quyền); thông lệ: bên nhận phải đủ năng lực.",
        "suggest": "Việc CĐT chuyển nhượng phải bảo đảm bên nhận đủ năng lực thanh toán và không làm giảm quyền lợi TVGS.",
    },
    # --- Chủ đề 25: Thông luật / quốc tế ---
    {
        ...
        "problem": "TVGS 'bồi hoàn và giữ CĐT vô hại' trước MỌI khiếu nại/tổn thất — phạm vi cực rộng, thường không trần, không loại trừ gián tiếp.",
        "basis": "Thông lệ tư vấn quốc tế giới hạn indemnity theo phần lỗi và có trần; BLDS chỉ bồi thường thiệt hại do lỗi.",
        "suggest": "Thêm trần trách nhiệm; loại trừ thiệt hại gián tiếp; giới hạn indemnity theo phần lỗi của TVGS và mức bảo hiểm.",
    },
    {
        "id": "best_endeavour",
        "topic": 25, "level": CAM,
    ...
    },
    {
        ...
        "problem": "Khi hợp đồng song ngữ lấy bản tiếng nước ngoài làm chuẩn, mọi khác biệt câu chữ sẽ giải thích bất lợi cho TVGS, nhất là khi tố tụng tại VN.",
    ...
    },
    {
        ...
        "problem": "Trao CĐT quyền quyết định tuyệt đối (vd novate hợp đồng, thay nhân sự) khiến TVGS không có cơ sở phản đối.",
    ...
    },
    {
        ...
        "problem": "(Phát hiện khi CÓ) — phụ lục PDPA quốc tế thường buộc TVGS xử lý/bảo vệ dữ liệu theo chuẩn CĐT kèm bồi thường vô hạn khi vi phạm.",
        "basis": "NĐ 13/2023/NĐ-CP về bảo vệ dữ liệu cá nhân.",
        "suggest": "Giới hạn phạm vi dữ liệu TVGS thực sự xử lý; gắn trách nhiệm với phần lỗi và mức bảo hiểm; tránh trách nhiệm vô hạn.",
    },
    {
        "id": "no_reduce_liability",
        "topic": 4, "level": CAM,
        "label": "'Phê duyệt của CĐT không làm giảm trách nhiệm TVGS'",
        ...
        "problem": "Câu này khiến việc CĐT/cơ quan đã chấp thuận vẫn không giúp TVGS giảm trách nhiệm — chấp nhận được nhưng cần giới hạn.",
        "basis": "Thông lệ giám sát: trách nhiệm gắn với phạm vi và mức lỗi.",
        "suggest": "Chấp nhận nhưng gắn trách nhiệm với phạm vi công việc và mức lỗi của TVGS, không phải trách nhiệm tuyệt đối.",
    },
    {
        ...
        "problem": "Buộc TVGS tiếp tục trách nhiệm/hỗ trợ sau thanh lý mà không thêm chi phí, kéo dài nghĩa vụ vô thời hạn.",
...
TOPICS = [
    {"id": 1, "name": "Định nghĩa & diễn giải",
     ...
     "missing_risk": CAM,
     "missing_note": "Không có mục định nghĩa/diễn giải — các thuật ngữ 'phạm vi', 'hoàn thành', 'chấp thuận' dễ bị hiểu rộng bất lợi cho TVGS."},
    ...
    {"id": 5, "name": "Thời gian, tiến độ, tạm ngừng",
     ...
     "missing_risk": CAM,
     "missing_note": "Không quy định rõ thời gian/tiến độ và cơ chế khi kéo dài ngoài lỗi TVGS."},
    ...
    {"id": 7, "name": "Tạm ứng, thu hồi, giữ lại",
     ...
     "missing_risk": CAM,
     "missing_note": "Không có điều khoản tạm ứng/giữ lại — bất lợi dòng tiền cho TVGS (mẫu TT02 thường có tạm ứng)."},
    ...
    {"id": 9, "name": "Bảo lãnh (thực hiện HĐ / hoàn trả tạm ứng)",
     ...
     "missing_risk": XANH,
     "missing_note": "Không yêu cầu bảo lãnh — thường có lợi cho TVGS (giảm chi phí), nhưng kiểm tra lại."},

    {"id": 10, "name": "Bảo hiểm",
     "keywords": ["bảo hiểm", "insurance", "trách nhiệm nghề nghiệp"],
     "missing_risk": XANH,
     "missing_note": "Không có yêu cầu bảo hiểm — thường ổn với TVGS, nhưng xác nhận lại."},

    {"id": 11, "name": "Nghĩa vụ & trách nhiệm Bên B (TVGS)",
    ...
    {"id": 12, "name": "Nghĩa vụ & chế tài đối với CĐT (đối xứng)",
     ...
     "missing_risk": DO,
     "missing_note": "Không thấy nghĩa vụ/chế tài đối với CĐT — mất đối xứng, TVGS không có công cụ khi CĐT chậm tài liệu/mặt bằng/thanh toán."},
    ...
    {"id": 14, "name": "Bản quyền / sở hữu trí tuệ",
     ...
     "missing_risk": CAM,
     "missing_note": "Không có điều khoản bản quyền/SHTT — kiểm tra để bảo lưu know-how của TVGS."},

    {"id": 15, "name": "Bảo mật thông tin",
     "keywords": ["bảo mật", "thông tin mật", "confidential", "non-disclosure", "nda"],
     "missing_risk": XANH,
     "missing_note": "Không có điều khoản bảo mật — thường không bất lợi cho TVGS, nhưng kiểm tra yêu cầu của dự án."},
...
EN_WARN_TERMS = {
    ...
    "defects notification period": "Thời gian thông báo khiếm khuyết (bảo hành) — TVGS có thể bị ràng buộc kéo dài đến hết giai đoạn này.",
    "final certificate": "Chứng chỉ cuối cùng — kiểm tra TVGS bị ràng buộc trách nhiệm đến thời điểm nào.",
